    ...
    return (
      <Layout>
        <div className="container mx-auto px-4 py-16 text-center">
          ...
          <h2 className="font-display text-2xl font-bold mb-2">Sign in to manage your store</h2>
    ...
    <Layout>
      <div className="container mx-auto px-4 py-8">
          ...
          <div className="flex items-center justify-between flex-wrap gap-4">
            <div>
              <h1 className="font-display text-3xl font-bold mb-1">
                <Store className="inline-block w-8 h-8 mr-2 -mt-1" />
                {vendor.business_name}
              </h1>
              <div className="flex items-center gap-2">
                <VendorStatusBadge status={vendor.status} />
                {vendor.onchain_verified && (
        ...
        {vendor.status === 'pending' && (
          <Card className="gradient-card border-warning/30 mb-8">
            <CardContent className="p-6 flex items-center gap-4">
              <Clock className="w-8 h-8 text-warning shrink-0" />
              <div>
                <h3 className="font-display font-semibold">Verification Pending</h3>
                <p className="text-sm text-muted-foreground">Your vendor account is being reviewed. You can add products, but they won't be visible until verified.</p>
              </div>
            </CardContent>
          </Card>
        )}
        ...
        <div className="mt-8">
          ...
          {products.length === 0 ? (
            <Card className="gradient-card border-border/50">
              <CardContent className="p-8 text-center text-muted-foreground">No products yet. Add your first product above!</CardContent>
            </Card>
          ) : (
              ...
              {products.map((product: any) => (
                <Card key={product.id} className="gradient-card border-border/50">
                  <CardContent className="p-4">
                    ...
                    <p className="font-display font-bold mt-1">{product.price_avax ?? 0} AVAX</p>
                  </CardContent>
                </Card>
              ))}
            </div>
    ...
    <Layout>
      <div className="container mx-auto px-4 py-8 max-w-xl">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          <h1 className="font-display text-3xl font-bold mb-2">Become a <span className="gradient-text">Vendor</span></h1>
          <p className="text-muted-foreground mb-8">Register your fitness business and start selling on FitConnect</p>
        </motion.div>
          ...
          <CardContent className="p-6">
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <Input value={form.businessName} onChange={(e) => setForm({ ...form, businessName: e.target.value })} placeholder="e.g. FitGear Kenya" className="mt-1" />
              </div>
              <div>
                <Label>Description</Label>
                <Textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} placeholder="Describe your products..." className="mt-1" />
              </div>
              <div>
              ...
              <div>
                ...
                <Input value={form.walletAddress} onChange={(e) => setForm({ ...form, walletAddress: e.target.value })} placeholder="0x..." className="mt-1" />
              </div>
              <div>
                <Label>Phone (for M-Pesa)</Label>
                <Input value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} placeholder="254712345678" className="mt-1" />
              </div>
...
function AddProductForm({ vendorId }: { vendorId: string }) {
  ...
  const [form, setForm] = useState({ name: '', description: '', category: 'equipment' as string, priceAvax: '', stock: '', imageUrl: '' });
  const [isSubmitting, setIsSubmitting] = useState(false);

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.name.trim() || !form.priceAvax) {
    ...
    try {
      const { error } = await supabase.from('products').insert({
        ...
        price_avax: parseFloat(form.priceAvax),
        stock: parseInt(form.stock) || 0,
        images: form.imageUrl ? [form.imageUrl] : [],
      });
      if (error) throw error;
      toast({ title: 'Product added!' });
      setForm({ name: '', description: '', category: 'equipment', priceAvax: '', stock: '', imageUrl: '' });
      setOpen(false);
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid sm:grid-cols-2 gap-4">
            <div>
              ...
              <Input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="e.g. Resistance Bands" className="mt-1" />
            </div>
            <div>
              ...
              <Select value={form.category} onValueChange={(v) => setForm({ ...form, category: v })}>
                ...
                <SelectContent>
                  <SelectItem value="gym_wear">Gym Wear</SelectItem>
          ...
          <div className="grid sm:grid-cols-3 gap-4">
            <div>
              ...
              <Input type="number" step="0.001" value={form.priceAvax} onChange={(e) => setForm({ ...form, priceAvax: e.target.value })} className="mt-1" />
            </div>
            <div>
              <Label>Stock</Label>
          ...
          <div className="flex gap-3">
            <Button type="submit" variant="hero" disabled={isSubmitting}>{isSubmitting ? 'Adding...' : 'Add Product'}</Button>
