...
const mockTrainer = {
  id: '1',
  name: 'James Mwangi',
  bio: 'Certified personal trainer with 8 years of experience in strength training and HIIT. I specialize in helping clients achieve their fitness goals through personalized workout plans and nutrition guidance.',
  ...
  reviews: [
    { id: '1', clientName: 'Alice K.', rating: 5, comment: 'Excellent trainer! James helped me lose 15kg in 3 months.', date: '2026-01-15' },
    { id: '2', clientName: 'Bob M.', rating: 5, comment: 'Very professional and motivating. Highly recommend.', date: '2026-01-10' },
    { id: '3', clientName: 'Carol N.', rating: 4, comment: 'Great sessions, learned proper form for all exercises.', date: '2026-01-05' },
  ],
};
...
export default function TrainerDetailPage() {
  ...
  const localAmount = rates ? convertToLocalCurrency(currentRate, 'USDC', selectedCountry, rates) : 0;
  ...
  const handleBook = async () => {
    ...
    try {
      ...
      toast.success('Booking confirmed!', {
        description: `Session with ${trainer.name} on ${format(selectedDate, 'PPP')} at ${selectedTime}. $${currentRate.toFixed(2)}`,
      });
  ...
  return (
    <Layout>
      <div className="container mx-auto px-4 py-8 max-w-6xl">
        <Link to="/trainers">
          <Button variant="ghost" className="mb-6 gap-2"><ArrowLeft className="w-4 h-4" />Back to Trainers</Button>
        </Link>
                    ...
                    <div className="flex-1">
                      <div className="flex items-start justify-between mb-3">
                        <div>
                          <div className="flex items-center gap-2 mb-1">
                            <h1 className="font-display text-2xl md:text-3xl font-bold">{trainer.name}</h1>
                            {trainer.isVerified && <CheckCircle className="w-6 h-6 text-success" />}
                          </div>
                      ...
                      <div className="flex flex-wrap items-center gap-4 mb-4">
                        <div className="flex items-center gap-1">
                          ...
                          <span className="text-muted-foreground">({trainer.reviews.length} reviews)</span>
                        </div>
                  ...
                  {trainer.isVerified && trainer.certificationHash && (
                    <div className="mt-4 p-3 rounded-lg bg-success/10 border border-success/20">
                      <div className="flex items-center justify-between">
                        ...
                        <a href={`https://ipfs.io/ipfs/${trainer.certificationHash}`} target="_blank" rel="noopener noreferrer" className="text-xs text-success hover:underline flex items-center gap-1">
              ...
              <Card className="gradient-card border-border/50">
                ...
                <CardContent className="space-y-4">
                  {trainer.reviews.map((review) => (
                    <div key={review.id} className="p-4 rounded-xl bg-muted/30 border border-border/30">
              ...
              <Card className="gradient-card border-border/50">
                <CardHeader>
                  <CardTitle className="font-display">
                    <div className="flex items-center justify-between"><span>Book Session</span></div>
                    <div className="grid grid-cols-2 gap-3 mt-3">
                      <div className="p-2 rounded-lg bg-muted/30 text-center">
                        <div className="text-xs text-muted-foreground flex items-center justify-center gap-1 mb-1">
                          <Users className="w-3 h-3" />Physical
                        </div>
                        <span className="gradient-text font-bold">${trainer.physicalRate.toFixed(2)}</span>
                        {rates && <p className="text-xs text-muted-foreground">≈ {formatLocalCurrency(physicalLocal, selectedCountry)}</p>}
                      </div>
                      <div className="p-2 rounded-lg bg-muted/30 text-center">
                        <div className="text-xs text-muted-foreground flex items-center justify-center gap-1 mb-1">
                          <Video className="w-3 h-3" />Virtual
                        </div>
                        <span className="gradient-text font-bold">${trainer.virtualRate.toFixed(2)}</span>
                        {rates && <p className="text-xs text-muted-foreground">≈ {formatLocalCurrency(virtualLocal, selectedCountry)}</p>}
                      </div>
                    </div>
                  </CardTitle>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <Button variant={sessionType === 'in-person' ? 'default' : 'outline'} onClick={() => setSessionType('in-person')} className="gap-2">
                        <Users className="w-4 h-4" />In-Person
                      </Button>
                      <Button variant={sessionType === 'virtual' ? 'default' : 'outline'} onClick={() => setSessionType('virtual')} className="gap-2">
                    ...
                    <div className="p-4 rounded-xl bg-muted/50 space-y-2">
                      ...
                      <div className="flex justify-between text-sm pt-2 border-t border-border/50">
                        ...
                        <div className="text-right">
                          <span className="font-bold">${currentRate.toFixed(2)}</span>
                          {rates && (
                  ...
                  {isAuthenticated ? (
                    <Button variant="hero" size="lg" className="w-full" disabled={!selectedDate || !selectedTime || isBooking} onClick={handleBook}>
                      {isBooking ? 'Confirming...' : 'Confirm Payment'}
                    </Button>
                  ) : (
                    <Button variant="hero" size="lg" className="w-full" onClick={login}>
                      Sign in to book
                    </Button>
                  )}

                  <p className="text-xs text-center text-muted-foreground">
                    15% platform fee included. Funds held securely until session completion.
                  </p>
                </CardContent>
              </Card>
            </motion.div>
