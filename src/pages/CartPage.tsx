          ...
          <div className="space-y-6">
                ...
                <Card key={item.id} className="gradient-card border-border/50">
                  <CardContent className="p-4 flex items-center gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="font-display font-bold text-sm mt-1">{item.product.price_avax ?? 0} AVAX</p>
                    </div>
                    <div className="flex items-center gap-2">
                      <Button variant="outline" size="icon" className="h-8 w-8" onClick={() => updateQuantity.mutate({ itemId: item.id, quantity: item.quantity - 1 })}>
                      ...
                      <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive" onClick={() => updateQuantity.mutate({ itemId: item.id, quantity: 0 })}>
            ...
            <Card className="gradient-card border-border/50">
              <CardContent className="p-6 space-y-4">
                ...
                <Button variant="ghost" className="w-full text-destructive" onClick={() => clearCart.mutate()}>
                  Clear Cart
                </Button>
              </CardContent>
            </Card>
